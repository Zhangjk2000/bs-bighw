...
export default {
  ...
  mounted() {
    this.list = this.filterList = this.imgList;
  ...
  methods: {
    ...
    handleSuccess(response) {
      ...
      getInfoApi().then((response) => {
        this.filterList = this.list = response.data['annotate_img_list'];
        this.$store.dispatch("user/setImgList", response.data['annotate_img_list']);
      });
    ...
    handleDelete(index) {
      ...
      for (let i = 0; i < this.list.length; i++) {
        if (this.list[i].id == id) {
          ...
          this.$store.dispatch("user/setImgList", this.list);
        }
      }
    },
  },
